...
{    
    ...
    if((substr(&rstamp(int($inf->{"timestamp"}/1000)),0,10) ne $curr_hour)&&($inf->{"action"} eq "get"))
    {
        return 0;
    }
...
sub fun_type1_op
{
    ...
    print($key."\t".$inf->{"timestamp"}."\t".$logsource."\t".$inf->{"action"}."\t".$inf->{"msg_type"}."\t".$inf->{"sender_sn"}."\t".$inf->{"size"}."\n");
}
