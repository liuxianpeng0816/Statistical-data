...
$hadoop fs -cat $root/total-usr/$day/* | awk -F'<|>' '{print $2}' | awk -F',' '{print $2}' | awk -F':' '{print "INSERT INTO CHAT_ACTIVE_USER_INFO_2013 (DATES, TYPES, DIME, VAL) VALUES ('\'''$day''\'', '\''TOTAL'\'', '\''ALL'\'', "$2") ;"}' >> $mr/sql_script/calculate_sql_$day.sql
