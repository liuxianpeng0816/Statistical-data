...
{    
    ...
    if(($inf->{"action"} ne "set")&&($inf->{"action"} ne "notify"))
    {
        return 0;
    }
    ...
    if(($inf->{"action"} eq "set")&&((substr($inf->{"sender_id"},0,3) eq "350")&&(length($inf->{"sender_id"}) == 10)||($inf->{"sender_id"} =~/[^0-9]+/)))
    {
        return 0;
    }
    ...
    if(($inf->{"action"} eq "set") &&( ($inf->{"size"} eq "")||($inf->{"size"} eq "null")))
    {
        $inf->{"size"} = 0;
    }
